...
const esTowns = {};
const newTownsData = townsData.map(town => {
    const slug = town.slug;
    esTowns[slug] = {
        description: town.description,
        longDescription: town.longDescription,
        services: town.services,
        features: town.features,
        testimonial: town.testimonial ? {
            quote: town.testimonial.quote,
            location: town.testimonial.location
        } : undefined
    };

    // transform town object to use keys
    const newTown = { ...town };
    newTown.description = `towns_data.${slug}.description`;
    newTown.longDescription = `towns_data.${slug}.longDescription`;
    newTown.services = town.services.map((_, i) => `towns_data.${slug}.services.${i}`);
    newTown.features = town.features.map((_, i) => `towns_data.${slug}.features.${i}`);
    if (town.testimonial) {
        newTown.testimonial = {
            quote: `towns_data.${slug}.testimonial.quote`,
            author: town.testimonial.author,
            location: `towns_data.${slug}.testimonial.location`
        };
    }
    return newTown;
});
...
esJson.towns_data = esTowns;
fs.writeFileSync(esJsonPath, JSON.stringify(esJson, null, 2));

console.log('Updated es.json');

// For en.json, we will just use the same keys but copy the Spanish text for now (user can translate later)
// actually the user wants me to translate. I am an AI. I should translate it.
// I will attempt simple translation logic here or later.
// For now, I will populate en.json with same content to avoid errors, and I will translate it in a separate step or just rely on my "Translate Site Pages" task description.
// I will output the translations for `en.json` to stdout so I can grab them and write to `en.json` myself using my brain to translate.

console.log('---EN_JSON_DATA---');
console.log(JSON.stringify(esTowns, null, 2));
console.log('---EN_JSON_END---');

console.log('---NEW_TOWNS_DATA---');
console.log(JSON.stringify(newTownsData, null, 2));
console.log('---NEW_TOWNS_END---');
